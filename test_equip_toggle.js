...
(function main() {
  ...
  console.log("# 4. Swap to an armor that has a nonzero Evasion modifier (for a meaningful toggle)");
  ...
  const asrch = q("#equip .srch[data-pick='aswap']");
  ok(!!asrch, "armor-swap search present");
  asrch.value = evaName; asrch.dispatchEvent(new win.Event("input", { bubbles: true }));
  const arow = q("#equip [data-pickadd='aswap']");
  ok(!!arow, "armor picker returned a row for '" + evaName + "'");
  click(arow);
  ok(A("armor.name") === evaName, "armor swapped to " + evaName);
  const a_eva = A("armor.eva"), a_agi = A("armor.agi") || 0, a_maj = A("armor.maj"),
        a_sev = A("armor.sev"), a_score = A("armor.score");
  ok(a_eva !== 0, "equipped armor has a nonzero Evasion contribution (" + a_eva + ")");
  // fresh snapshot with this armor equipped
  const E = A("evasion"), M = A("majorBase"), S = A("severeBase"), SC = A("armorScore"),
        AG = A("traits.Agility");

  console.log("# 5. Armor unequip removes every armor contribution");
  const acb = q("#equip [data-armequip]");
  ok(!!acb, "armor equip checkbox present");
  acb.checked = false; change(acb);
  ok(A("armor.equipped") === false, "armor now unequipped");
  ...
  ok(/unequipped/.test(q("#equip").innerHTML), "panel flags the armor as unequipped");

  console.log("# 6. Re-equip restores every stored stat EXACTLY (lossless — SUBSTAT-safe)");
  const acb2 = q("#equip [data-armequip]");
  acb2.checked = true; change(acb2);
  ok(A("armor.equipped") === true, "armor re-equipped");
  ...
  const acb3 = q("#equip [data-armequip]"); acb3.checked = false; change(acb3);   // also leave armor off
