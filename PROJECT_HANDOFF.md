...
| **#10** | 🟡 Feature (STEP 1 DRAFTED 2026-07-11, `d0cc41a`) | **Marketplace / Tinker (buy & sell values)** | Searchable list of everything purchasable, showing worth as a **range** — e.g. *"Short Sword · common · 1–3 handfuls · used value 1 handful."* **Step 1 (data model) is drafted for review in `MARKETPLACE_PRICE_MODEL_DRAFT.md`** (4 rarity buckets + tier×rarity price table + all 120 items/consumables classified; nothing applied to the equipment JSON yet). **Awaiting Chance's approval of the draft** before the data is written and the UI (step 2) is built. |
