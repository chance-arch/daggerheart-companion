...
| **#16** | Feature — **OPEN** (logged 2026-07-19) | **Search Everything needs a "show all" option** | Today the 🔎 Search All panel hard-caps results at **6 per category** (`CAP=6` in `resultsHTML`, ~line 1288) and the "+N more — narrow your search" line is dead text; an empty box shows only a "type to search" hint, so there's no way to browse a full list. Wanted: a way to see all matches — most naturally, make "+N more" a click-to-expand per category (and consider a browse-everything view for the empty-box state). Implementation note: expand **per category on demand** rather than mounting every catalog at once, to stay inside the §4a "never mount the whole library" pattern that keeps the app fast. |
