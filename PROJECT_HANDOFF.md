...
| **#17** | ✅ Bug + audit (BUILT 2026-07-19) | **Corrosive Projectile damage wrong → full 189-card audit vs the book** | Chance caught Corrosive Projectile dealing "6d4" — the book says **d6+4**. Root cause: **daggerheartsrd.com (our rank-4 scrape source) garbled the damage notation on several cards**, so our data matched the *site* perfectly while contradicting the *book* — auditing against our own source would have found nothing. Ran a **full-text audit of all 189 cards** against an independent conversion of the official SRD PDF ([seansbox/daggerheart-srd](https://github.com/seansbox/daggerheart-srd)), tie-breaking disagreements via Demiplane (rank 7) + daggerheart.su: **21 diffs → 17 cards corrected, 4 were the comparison source's own OCR typos (ours kept)**. Then (same day, Chance's ask) a **second, three-way verification pass** added an independent third lineage ([daggersearch/daggerheart-data](https://github.com/daggersearch/daggerheart-data)) across all 189 cards: **every one of the 17 corrections independently confirmed**, and the pass caught an **18th error** — **Book of Vagras** Reveal is "anything magically hidden within Close range **the roll would succeed against**" (both our scrape AND seansbox had dropped the clause; daggersearch + daggerheart.su + Demiplane/Core-p.149 agree 3-lineages-to-2). Residual three-way diffs are formatting-only (list numerals, threshold phrasing, quote style). Damage-notation class fixed on 6 cards (Corrosive d6+4, Telekinesis d12+4 + missing "This spell then ends.", Bolt Beacon d8+2, Book of Norai d20+5, Preservation Blast d8+3, Rain of Blades d8+2) with mechanics dice re-encoded `{count:1}` so Proficiency scales the count; plus Know Thy Enemy (**"tactics"**, and Stress removes a **Fear** — not a Stress — from the GM's Fear Pool), Bare Bones Tier-4 thresholds **15/38**, Enrapture recall **0**, Forager "A unique food", Tava's Armor, Parallela, and 5 smaller wording fixes. Applied to **all five copies**: annotated JSON, text-only JSON, app CARDS embed, SEED_THERON, and the `.xlsx` review twin; DATA_INDEX regenerated. **New: `refreshCardCopies()` migration** — saved characters store card copies *by value*, so corrections never reached existing rosters; now every boot + cloud adopt refreshes stored copies' library fields from CARDS by name (runtime keys and homebrew names untouched), and the healed roster re-saves → cloud-syncs. jsdom-verified (`test_card_refresh.js`, 10 checks; full suite 12 files green). **Two things for Chance:** (a) six cards now deal book-correct damage at the table — numbers players see will change; (b) *open design question:* Preservation Blast's book text says "d8+3 using your **Spellcast trait**" — the interpreter can't scale dice count by a trait, so it rolls a flat 1d8+3 with the wording shown; teaching the interpreter `scaleBy:"spellcastTrait"` is a small design-gated follow-up if wanted. |
| **#16** | ✅ Feature (BUILT 2026-07-20) | **Search Everything → full reference browser** | Chance's spec (superseding the earlier "show all" note): the panel's purpose is to **fully reference anything in the game** — all info on any card, weapon, etc.; the "+N more" block was a usability blocker; results must scroll; every result must click-to-expand. Rebuilt the 🔎 Search All panel: **(a)** results container scrolls (`max-height:min(62vh,540px)`, scroll position preserved across re-renders); **(b)** the dead "+N more — narrow your search" text is replaced by a working per-category **"Show all N" / "Show first 6"** toggle — expansion stays per-category on demand, honoring §4a's "never mount the whole library" rule; **(c)** **every row click-expands to the record's complete entry** — cards show domain/level/recall/coverage classification + full rules text, weapons show tier/category/trait/range/damage/burden/feature, armor shows thresholds/score/modifiers, items & consumables show rarity + full description, beastforms show the whole statblock (trait, evasion, attack, advantages, features), conditions show full text; clicks *inside* an expanded entry don't collapse it (text is selectable); **(d)** an **empty box = browse mode** listing all ~570 entries by category (capped 6 + Show all each). Expansion state resets on a new term. jsdom-verified (`test_search_overhaul.js`, 22 checks; existing `test_armor_list_and_search.js` untouched and green; full suite 13 files). **Look/feel needs Chance's eyes.** |
