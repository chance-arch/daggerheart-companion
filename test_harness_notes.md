...
| `test_beastform_ux.js` | Field note #7: Beastform assume/drop UX — the Combat Actions quick-launcher (tier-grouped select + Assume/Evolution), in-panel Drop button, and the in-app Evolution trait picker that replaces the native `prompt()` (assume marks 1 Stress; drop exits; evolution stages a pending choice then spends exactly 3 Hope on the chosen trait; cancel is clean; buttons disable on insufficient Stress/Hope; `prompt()` never called). Drives the real UI; reads state via `App.active._session`. 31 checks. |
